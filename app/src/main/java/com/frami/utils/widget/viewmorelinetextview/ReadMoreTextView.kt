...
class ReadMoreTextView @JvmOverloads constructor(
    ...
    private var readMoreColor = ContextCompat.getColor(context, R.color.colorHeaderText)
...
//        setupListener()
    ...
    private fun setupAttributes(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        val typedArray =
            context.obtainStyledAttributes(
                attrs,
                R.styleable.ReadMoreTextViewByLine,
                defStyleAttr,
                0
            )

        readMoreMaxLine =
            typedArray.getInt(R.styleable.ReadMoreTextViewByLine_readMoreMaxLine, readMoreMaxLine)
        readMoreText =
            typedArray.getString(R.styleable.ReadMoreTextViewByLine_readMoreText) ?: readMoreText
        readMoreColor =
            typedArray.getColor(R.styleable.ReadMoreTextViewByLine_readMoreColor, readMoreColor)
        typedArray.recycle()
    }
    ...
    fun isTextEmpty(): Boolean {
        return text == null || text.toString().trim { it <= ' ' }.isEmpty()
    }
}