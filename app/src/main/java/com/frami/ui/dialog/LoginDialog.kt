...
class LoginDialog(
    ...
    private var bindingSheet: DialogLoginBinding? = null
    ...
    init {
        this.bindingSheet = DataBindingUtil.inflate<DialogLoginBinding>(
            this.layoutInflater,
    ...
    private fun clickListener() {
        ...
        bindingSheet!!.tvForgotPassword.setOnClickListener {
            if (b2cApp == null) {
                return@setOnClickListener;
            }
            requestLogin(BuildConfig.B2C_FORGOT_PASSWORD)
        }
    }
...
//            val accountId = account.find { it.authority == getAuthorityFromPolicyName(policy) }
//
//            val parametersSilent = AcquireTokenSilentParameters.Builder()
//                .fromAuthority(getAuthorityFromPolicyName(policy))//Pass User object policy
//                .withScopes(B2CConfiguration.scopes)
...
//            val accountId2 = account2.find { it.authority == getAuthorityFromPolicyName(policy) }
//            val parametersOnError = AcquireTokenParameters.Builder()
//                .startAuthorizationFromActivity(mActivity)
//                .withScopes(B2CConfiguration.scopes)
//                .forAccount(accountId2)
//                .withCallback(authInteractiveCallback)
//                .build()
